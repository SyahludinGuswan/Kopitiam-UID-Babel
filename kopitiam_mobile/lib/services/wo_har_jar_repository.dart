...
class WoHarJarRepository {
  ...
  Future<List<WoHarJar>> semua() async {
    ...
    final rows = await db.rawQuery(
      "SELECT * FROM ${DatabaseHelper.woHarJarTable} ORDER BY CASE status_wo WHEN '${WoHarJar.statusMenunggu}' THEN 0 WHEN '${WoHarJar.statusSedang}' THEN 1 ELSE 2 END, kode_wo DESC",
    );
  ...
  Future<int> jumlahBelumSinkron() async {
    ...
    final result = await db.rawQuery(
      "SELECT COUNT(*) AS total FROM ${DatabaseHelper.woHarJarTable} WHERE is_synced = 0 AND status_wo = '${WoHarJar.statusSelesai}'",
    );
    ...
    if (response['success'] == true &&
      ...
      if (totalSheet > 0 && rejectedByUlp > 0)
        return WoSyncResult(
          ...
          diproses: 0,
          pesan:
              'Ditemukan $totalSheet data WO Har Jar di server, tetapi $rejectedByUlp tidak cocok dengan Kode ULP akun Anda (${response['kodeUlpFilter'] ?? ''} vs ${response['sampleKodeUlp'] ?? ''} pada data). Hubungi admin.',
        );
    }
  ...
  String generateKodeMaterial(String kodeWo) =>
      'MAT-$kodeWo-${DateTime.now().millisecondsSinceEpoch}${Random().nextInt(900) + 100}';
  ...
  Future<WoSyncResult> sinkron(String token) async {
    ...
    return WoSyncResult(
      ...
      pesan: processed == rows.length
          ? null
          : '$processed dari ${rows.length} WO Har Jar berhasil. ${firstError ?? 'WO gagal tetap disimpan untuk retry.'}',
    );
  }
}
