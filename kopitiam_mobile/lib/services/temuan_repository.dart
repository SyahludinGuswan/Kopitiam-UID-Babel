...
class TemuanRepository {
  ...
  Future<Map<String, dynamic>> _send(
    ...
    Map<String, dynamic> payload,
      ...
      : _sender(token, payload);

  Future<Database> _database() async {
    final db = database ?? await _db.database;
    await db.execute(
      '''CREATE TABLE IF NOT EXISTS temuan_inspeksi (
    ...
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_temuan_c4a_queue ON temuan_inspeksi(kode_wo,is_dirty,sync_status)',
    );
    return db;
  }
  ...
  Future<void> _addColumn(Database db, String name, String definition) async {
    ...
    if (!info.any((row) => '${row['name']}'.toLowerCase() == name))
      await db.execute(
        'ALTER TABLE temuan_inspeksi ADD COLUMN $name $definition',
      );
  }
  ...
  Future<String> kodeBaru(String kodeWo) async {
    ...
    final rows = await db.rawQuery(
      'SELECT kode_temuan FROM temuan_inspeksi WHERE kode_wo = ? ORDER BY kode_temuan DESC LIMIT 1',
      [kodeWo],
    );
  ...
  Future<TemuanInspeksi> _preparePhotos(TemuanInspeksi item) async {
    ...
    final a = item.kodeWo.isNotEmpty && _sudahWatermark(item.fotoTemuan)
        ? item.fotoTemuan
    ...
    final b = item.kodeWo.isNotEmpty && _sudahWatermark(item.fotoLingkungan)
        ? item.fotoLingkungan
        : await PhotoWatermarkService.render(
    ...
    final primary = await _persistPhoto(a, item.kodeTemuan, 'Foto Temuan'),
        environment = await _persistPhoto(
          b,
          item.kodeTemuan,
          'Foto Lingkungan',
        );
    return TemuanInspeksi.fromMap({
      ...item.toMap(),
      'foto_temuan': primary.path,
      'foto_lingkungan': environment.path,
    });
  }
  ...
  Future<File> _persistPhoto(
    ...
    final root = await _db.accountDocumentsDirectory(),
        folder = Directory(p.join(root.path, 'temuan_photos', _safeName(code)));
    await folder.create(recursive: true);
    final match = RegExp(
      r'(\d{6})(?=\.jpe?g$)',
      caseSensitive: false,
    ).firstMatch(p.basename(source.path));
    final stamp =
        match?.group(1) ??
        DateTime.now()
            .toIso8601String()
    ...
    final destination = File(
      p.join(folder.path, '${_safeName(code)}.$label.$stamp.jpg'),
    );
    return p.equals(source.path, destination.path)
        ? source
  ...
  Future<Map<String, dynamic>> _payload(TemuanInspeksi item) async {
    ...
    final primary = File(item.fotoTemuan),
        environment = File(item.fotoLingkungan);
  ...
  Future<C4aSyncResult> sinkronC4a(
    String token, {
    bool hanyaGagal = false,
  }) async {
    final db = await _database();
    final rows = await db.query(
      'temuan_inspeksi',
      ...
      whereArgs: hanyaGagal
          ...
          : [
              TemuanInspeksi.statusDraft,
              TemuanInspeksi.statusQueued,
              TemuanInspeksi.statusFailed,
              TemuanInspeksi.statusSending,
            ],
    ...
    for (final row in rows) {
      final item = TemuanInspeksi.fromMap(row),
          attempt = DateTime.now().toUtc().toIso8601String();
      await db.update(
        'temuan_inspeksi',
        {
      ...
      try {
        final response = await _send(token, await _payload(item));
        if (response['success'] != true)
          throw StateError('${response['message'] ?? 'Sinkronisasi gagal.'}');
        await db.update(
          'temuan_inspeksi',
          {
            'is_dirty': 0,
  ...
  Future<bool> kirimUlangC4a(String token, String code) async {
    ...
    await db.update(
      'temuan_inspeksi',
      {'sync_status': TemuanInspeksi.statusQueued, 'sync_error': ''},
      where: "kode_temuan = ? AND kode_wo = ''",
      whereArgs: [code],
    );
    final rows = await db.query(
      'temuan_inspeksi',
  ...
  Future<void> sinkron(String token) async {
    ...
    for (final row in rows) {
      final item = TemuanInspeksi.fromMap(row),
          primary = File(TemuanInspeksi.fromMap(row).fotoTemuan),
          environment = File(TemuanInspeksi.fromMap(row).fotoLingkungan);
      await _validatePhoto(primary);
      await _validatePhoto(environment);
      ...
      if (response['success'] != true)
        throw StateError(
          '${response['message'] ?? 'Sinkronisasi foto gagal.'}',
        );
      await db.update(
        'temuan_inspeksi',
        {
  ...
  void validateC4a(TemuanInspeksi item) {
    ...
    if (isRowC4a(item.temuan) &&
        (item.jenisPohon.trim().isEmpty ||
            prioritasC4a(item.temuan, item.jarak, item.tinggiPohon, const []) !=
                item.prioritas))
      throw StateError(
  ...
  String prioritasC4a(
    ...
    final values = master
        .where((r) => _same(_find(r, ['Temuan']), temuan))
        .map((r) => _find(r, ['Prioritas']))
  ...
  String prioritas(
    ...
    if (isRowC4a(temuan)) {
      final d = jarak ?? 0, h = tinggi ?? 0;
      if (h < 9) return d > 5 ? 'Minor' : 'Mayor';
      if (d < 3) return 'Mayor';
      if (d < 6) return 'Sedang';
      return 'Minor';
    }
    for (final row in master) {
      final n = _find(row, const ['Temuan', 'Nama Temuan']);
      if (n.isNotEmpty && _same(n, temuan))
        return _find(row, const ['Prioritas']);
    }
    return '';
  }

  static String _find(Map<String, dynamic> row, List<String> keys) {
    String n(String k) => k.toLowerCase().replaceAll(RegExp(r'[^a-z0-9]'), '');
    for (final k in keys) {
      final v = '${row[k] ?? ''}'.trim();
      if (v.isNotEmpty) return v;
    }
    final set = keys.map(n).toSet();
    for (final e in row.entries)
      if (set.contains(n(e.key)) && '${e.value ?? ''}'.trim().isNotEmpty)
        return '${e.value}'.trim();
    return '';
  }
  ...
  static String _folder(String ulp, String object, String leaf, DateTime now) {
    ...
    return 'Kopitiam/Rekap Temuan Inspeksi/$ulp/$object/${now.year}/${now.month.toString().padLeft(2, '0')}. ${m[now.month - 1]}/${now.day.toString().padLeft(2, '0')}/$leaf/';
  }
}
...
class C4aSyncService {
  ...
  Future<void> flush({void Function(C4aSyncResult result)? onResult}) async {
    ...
    _running = true;
    try {
      onResult?.call(await repository.sinkronC4a(token));
    } finally {
      _running = false;
    }
  }
  ...
  static Future<bool> _hasNetwork() async {
    try {
      final r = await InternetAddress.lookup('script.google.com')
