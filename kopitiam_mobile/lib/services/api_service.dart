...
class ApiService {
  ...
  static Future<http.Response> _postAppsScript(
    ...
    try {
      ...
      if (_redirectCodes.contains(contentResponse.statusCode))
        throw StateError(
          'Redirect ContentService berulang (HTTP ${contentResponse.statusCode}). Perbarui deployment Apps Script.',
        );
  ...
  static Future<Map<String, dynamic>> _sendWithBackoff(
    Map<String, dynamic> payload,
  ) async {
    var retryCount = 0;
    while (true) {
  ...
  static Future<Map<String, dynamic>> _postMap(Map<String, dynamic> payload) {
    final action = '${payload['action'] ?? ''}';
    return ApiActivity.track(
      action,
      () => action.startsWith('sync')
          ? _syncCoordinator.run(() => _sendWithBackoff(payload))
          : _sendWithBackoff(payload),
    );
  }
  ...
  static Future<Map<String, dynamic>> _syncRows(
    ...
    for (final source in sourceRows) {
      ...
      if (requestBytes > _maximumSyncRequestBytes) {
        final state = await SyncFailureStore.recordFailure(
          ...
          kodeWo: code,
          error:
              'Ukuran request $requestBytes byte melebihi batas aman 12 MiB.',
      ...
      for (var attempt = 0; attempt <= _perWoRetryDelays.length; attempt++) {
        try {
          ...
          lastError =
              response['message'] ?? response['kode'] ?? 'Receipt tidak valid.';
    ...
    if (failures.isNotEmpty)
      return {
        ...
        'kode': 'SYNC_PARTIAL',
        'message':
            '${receipts.length} WO berhasil; ${failures.length} WO tetap tersimpan untuk tindak lanjut.',
  ...
  static Future<Map<String, dynamic>> loginPerangkat(
    ...
    if (response['success'] == true && response['deviceToken'] != null) {
      if (DeviceSessionService.normalizeUsername(response['username']) !=
          DeviceSessionService.normalizeUsername(username)) {
  ...
  ) => _syncRows('syncWoInsjar', token, rows);
  ...
  static Future<Map<String, dynamic>> syncWoRow(
    String token,
    List<Map<String, dynamic>> rows,
  ) => _syncRows('syncWoRow', token, rows);
  ...
  static Future<Map<String, dynamic>> syncWoHarJar(
    String token,
    List<Map<String, dynamic>> rows,
  ) => _syncRows('syncWoHarJar', token, rows);
  ...
  static Future<Map<String, dynamic>> syncWoHarDu(
    String token,
    List<Map<String, dynamic>> rows,
  ) => _syncRows('syncWoHarDu', token, rows);
  ...
  static Future<Map<String, dynamic>> syncWoInsdu(
    String token,
    List<Map<String, dynamic>> rows,
  ) => _syncRows('syncWoInsdu', token, rows);
