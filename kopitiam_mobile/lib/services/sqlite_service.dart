...
class SqliteService {
  ...
  Future<void> _createC4aQueueSchema(DatabaseExecutor db) async {
    ...
    await db.execute(
      'CREATE INDEX idx_temuan_c4a_queue ON temuan_inspeksi(kode_wo,is_dirty,sync_status)',
    );
  }

  Future<void> _createMasterSchema(DatabaseExecutor db) async {
    await db.execute(
      """CREATE TABLE master_data_rows (id INTEGER PRIMARY KEY AUTOINCREMENT, dataset TEXT NOT NULL, row_key TEXT NOT NULL, payload_json TEXT NOT NULL, synced_at TEXT NOT NULL, UNIQUE(dataset,row_key))""",
    );
    await db.execute(
      'CREATE INDEX idx_master_dataset ON master_data_rows(dataset)',
    );
  }

  Future<void> _createWoSchema(DatabaseExecutor db) async {
    await db.execute(
      """CREATE TABLE wo_insjar (
    ...
    await db.execute(
      'CREATE INDEX idx_wo_insjar_tanggal ON wo_insjar(tanggal)',
    );
    await db.execute(
      'CREATE INDEX idx_wo_insjar_status ON wo_insjar(status_wo)',
    );
