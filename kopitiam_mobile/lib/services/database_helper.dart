...
/// Baseline SQLite offline-first Kopitiam untuk modul WO mobile.
class DatabaseHelper {
  ...
  static Future<void> createHarJarSchema(DatabaseExecutor db) async {
    await db.execute('''CREATE TABLE $woHarJarTable (
    ...
    await db.execute('CREATE INDEX idx_wo_har_jar_status ON $woHarJarTable(status_wo)');
    await db.execute('CREATE INDEX idx_wo_har_jar_synced ON $woHarJarTable(is_synced)');

    await db.execute('''CREATE TABLE $woHarDuTable (
      id INTEGER PRIMARY KEY AUTOINCREMENT, no TEXT NOT NULL DEFAULT '', kode_wo TEXT NOT NULL UNIQUE,
      kode_temuan TEXT NOT NULL DEFAULT '', kode_uiw TEXT NOT NULL DEFAULT '', kode_up3 TEXT NOT NULL DEFAULT '',
      kode_ulp TEXT NOT NULL DEFAULT '', ulp TEXT NOT NULL DEFAULT '', hari TEXT NOT NULL DEFAULT '', tanggal TEXT NOT NULL DEFAULT '',
    ...
    await db.execute('CREATE INDEX idx_wo_har_du_status ON $woHarDuTable(status_wo)');
    await db.execute('CREATE INDEX idx_wo_har_du_synced ON $woHarDuTable(is_synced)');

    await db.execute('''CREATE TABLE $woMaterialHarJarTable (
    ...
    await db.execute('CREATE INDEX idx_wo_material_kode_wo ON $woMaterialHarJarTable(kode_wo)');
  }

  static Future<void> createInsduSchema(DatabaseExecutor db) async {
    await db.execute('''CREATE TABLE $woInsduTable (
      id INTEGER PRIMARY KEY AUTOINCREMENT, no TEXT NOT NULL DEFAULT '', kode_wo TEXT NOT NULL UNIQUE,
      ...
      jurusan_terpasang INTEGER, jurusan_terpakai INTEGER, kapasitas INTEGER, arus_maksimal_per_fasa REAL,
      ...
      koordinat_penginputan_wbp TEXT NOT NULL DEFAULT '', waktu_penginputan_wbp TEXT NOT NULL DEFAULT '', jarak_gardu_petugas_wbp REAL,
      koordinat_penginputan_lwbp TEXT NOT NULL DEFAULT '', waktu_penginputan_lwbp TEXT NOT NULL DEFAULT '', jarak_gardu_petugas_lwbp REAL,
    ...
    await db.execute('CREATE INDEX idx_wo_insdu_status ON $woInsduTable(status_wo)');
    await db.execute('CREATE INDEX idx_wo_insdu_dirty ON $woInsduTable(is_dirty)');
  }

  static Future<void> createYandalP0Schema(DatabaseExecutor db) async {
    await db.execute('''CREATE TABLE $woYandalP0Table (
      id INTEGER PRIMARY KEY AUTOINCREMENT, no TEXT NOT NULL DEFAULT '', kode_wo TEXT NOT NULL UNIQUE,
      ...
      folder_path TEXT NOT NULL DEFAULT '', is_synced INTEGER NOT NULL DEFAULT 0)''');
    await db.execute('CREATE INDEX idx_wo_yandal_p0_status ON $woYandalP0Table(status_wo)');
    await db.execute('CREATE INDEX idx_wo_yandal_p0_synced ON $woYandalP0Table(is_synced)');
  }
}
