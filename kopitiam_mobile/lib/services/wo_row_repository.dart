...
class WoRowRepository {
  ...
  Future<List<WoRow>> semua() async {
    ...
    final rows = await db.rawQuery("SELECT * FROM wo_row ORDER BY CASE status_wo WHEN 'Penugasan Tim' THEN 0 WHEN 'Progress Pekerjaan' THEN 1 ELSE 2 END, kode_wo DESC");
    return rows.map(WoRow.fromMap).toList();
  }

  Future<int> jumlahDirty() async {
    final db = await _database();
    final result = await db.rawQuery("SELECT COUNT(*) AS total FROM wo_row WHERE is_dirty = 1 AND status_wo = 'Selesai'");
    return (result.first['total'] as int?) ?? 0;
  }

  Future<WoRow?> cari(String kodeWo) async {
    final db = await _database();
    final rows = await db.query('wo_row', where: 'kode_wo = ?', whereArgs: [kodeWo], limit: 1);
  ...
  Future<WoSyncResult> download(String token) async {
    ...
    if (response['success'] == true && pendingList is List && pendingList.isEmpty) {
      ...
      if (totalSheet > 0 && rejectedByUlp > 0) {
        ...
        return WoSyncResult(total: 0, diproses: 0, pesan: 'Ditemukan $totalSheet data ROW di server, tetapi $rejectedByUlp tidak cocok dengan Kode ULP akun Anda ($filter vs $sample pada data). Hubungi admin untuk memeriksa Kode ULP akun atau data ROW.');
      }
    }
    if (response['success'] != true || pendingList is! List) {
      return WoSyncResult(total: 0, diproses: 0, pesan: (response['message'] ?? 'Download ROW gagal.').toString());
    }
    final db = await _database();
    ...
    await db.transaction((txn) async {
      for (final row in pendingList) {
        ...
        if (WoRow.normalisasiStatus(item.statusWo) == WoRow.statusSelesai) continue;
        final inserted = await txn.insert('wo_row', item.toMap()..['synced_at'] = now, conflictAlgorithm: ConflictAlgorithm.ignore);
        if (inserted > 0) added++;
      }
      await txn.insert('sync_metadata', {'key': 'WO_ROW', 'synced_at': now, 'row_count': added, 'status': 'success', 'error_message': ''}, conflictAlgorithm: ConflictAlgorithm.replace);
    });
  ...
  Future<void> mulaiPekerjaan(String kodeWo) async {
    final db = await _database();
    await db.update('wo_row', {'status_wo': WoRow.statusProgress, 'is_dirty': 0, 'waktu_input': WoInsjar.stampLengkap(DateTime.now())}, where: 'kode_wo = ?', whereArgs: [kodeWo]);
  }
  ...
  Future<void> simpan(WoRow item) async {
    ...
    final db = await _database();
    await db.insert('wo_row', item.copyWith(fotoSesudah: photoPath.path, isDirty: true).toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  TemuanInspeksi _watermarkItem(WoRow item) => TemuanInspeksi(kodeTemuan: item.kodeTemuan, kodeWo: item.kodeWo, temuan: item.temuan, jenisObject: item.jenisObject, tier: item.tier, prioritas: item.prioritas, koordinat: item.koordinat, ulp: item.ulp, penyulang: item.penyulang, section: item.section, segmen: item.segmen, hari: item.hari, tanggal: item.tanggal, waktuInput: item.waktuInput);
  String _safeName(String value) => value.replaceAll(RegExp(r'[^A-Za-z0-9._-]'), '_');

  Future<WoSyncResult> sinkron(String token) async {
    final db = await _database();
    final rows = await db.query('wo_row', where: "is_dirty = 1 AND status_wo = '${WoRow.statusSelesai}'", orderBy: 'kode_wo ASC');
    if (rows.isEmpty) return const WoSyncResult(total: 0, diproses: 0, pesan: 'Belum ada ROW selesai yang siap disinkronkan.');
    var processed = 0;
    String? firstError;
    for (final row in rows) {
      ...
      try {
        ...
        await db.delete('wo_row', where: 'kode_wo = ?', whereArgs: [item.kodeWo]);
        processed++;
      } catch (error) {
        firstError ??= '$error';
      }
    }
    return WoSyncResult(total: rows.length, diproses: processed, pesan: processed == rows.length ? null : '$processed dari ${rows.length} ROW berhasil. ${firstError ?? 'WO gagal tetap disimpan untuk retry.'}');
  }
}
