...
import 'insdu_capacity_guard.dart';
...
class _WoInsduFormScreenState extends State<WoInsduFormScreen> {
  ...
  static const wbpLoad = {
    'bebanUtamaRWbp': 'FASA R',
    'bebanUtamaSWbp': 'FASA S',
    'bebanUtamaTWbp': 'FASA T',
    'bebanJurusanNWbp': 'NETRAL N',
  };
  static const wbpVoltage = {
    'teganganRswbp': 'R-S',
    'teganganStwbp': 'S-T',
    'teganganRtwbp': 'R-T',
    'teganganRnwbp': 'R-N',
  ...
  static const lwbpLoad = {
    'bebanUtamaRLwbp': 'FASA R',
    'bebanUtamaSLwbp': 'FASA S',
    'bebanUtamaTLwbp': 'FASA T',
    'bebanJurusanNLwbp': 'NETRAL N',
  };
  static const lwbpVoltage = {
    'teganganRslwbp': 'R-S',
    'teganganStlwbp': 'S-T',
    'teganganRtlwbp': 'R-T',
    'teganganRnlwbp': 'R-N',
  ...
  Iterable<String> get _numericKeys => [
        ...wbpLoad.keys,
        ...wbpVoltage.keys,
        ...lwbpLoad.keys,
        ...lwbpVoltage.keys,
  ...
  void initState() {
    super.initState();
    _capacity = WoInsdu.kapasitasOptions.contains(wo.kapasitas)
        ? wo.kapasitas
        : null;
    ...
    for (final entry in values.entries) {
      ...
      _fields[entry.key] = TextEditingController(
        text: _numericKeys.contains(entry.key)
            ? raw.replaceAll('.', ',')
            : raw,
      );
    }
  ...
  List<double>? get _garduCoordinate {
    final raw = wo.koordinatGardu.trim().isNotEmpty
        ? wo.koordinatGardu
        : '${wo.lat},${wo.long}';
  ...
  bool get _hasDecimalError =>
      _numericKeys.any((key) => _decimalError(key) != null);
  bool get _complete =>
      _capacity != null &&
      ...
      _lwbpFix != null &&
      _wbpTime.isNotEmpty &&
      _lwbpTime.isNotEmpty;

  Map<String, double?> get _currentValues => {
        for (final key in [...wbpLoad.keys, ...lwbpLoad.keys])
          key: _number(key),
      };

  Future<void> _capture(bool wbp) async {
    final blocked = wbp
        ? _wbpFix != null || _gettingWbp
        : _lwbpFix != null || _gettingLwbp;
    ...
    try {
      final fix = await HighAccuracyLocationService.acquire(
        onSample: (_, accuracy) {
          ...
          setState(() {
            if (wbp) {
              _wbpAccuracy = accuracy;
            } else {
              _lwbpAccuracy = accuracy;
            }
          });
        },
      );
  ...
  WoInsdu _buildItem({required bool finish}) {
    final now = DateTime.now();
    final start = WoInsjar.parseStamp(wo.waktuMulai) ??
        WoInsjar.parseStamp(_wbpTime) ??
        now;
    ...
    return WoInsdu(
      ...
      kapasitas: capacity,
      arusMaksimalPerFasa:
          capacity == null ? null : WoInsdu.maximumPhaseCurrent(capacity),
      ...
      koordinatPenginputanWbp: _wbpFix?.coordinate ?? '',
      waktuPenginputanWbp: _wbpTime,
      jarakGarduPetugasWbp: _wbpDistance,
      koordinatPenginputanLwbp: _lwbpFix?.coordinate ?? '',
      waktuPenginputanLwbp: _lwbpTime,
      ...
      waktuMulai:
          wo.waktuMulai.isEmpty ? WoInsjar.stampLengkap(start) : wo.waktuMulai,
      waktuSelesai:
          finish ? WoInsjar.stampLengkap(now) : wo.waktuSelesai,
      durasiPekerjaan:
          finish ? WoInsjar.hitungDurasi(start, now) : wo.durasiPekerjaan,
      statusWo: finish ? WoInsdu.statusSelesai : WoInsdu.statusDalam,
      isDirty: finish,
    );
  }
  ...
  Future<void> _savePressed() async {
    ...
    if (_hasDecimalError) {
      setState(() {});
      _message(
        'Perbaiki kolom merah. Gunakan (,) sebagai pemisah.',
        error: true,
      );
      return;
    }
    ...
    final candidate = _buildItem(finish: true);
    try {
      WoInsduRepository.validateContract(candidate, requireComplete: true);
    } catch (error) {
    ...
    final capacity = _capacity!;
    final warnings = InsduCapacityGuard.evaluate(
      capacity: capacity,
      values: _currentValues,
    );
    final confirmed = await InsduCapacityGuard.confirm(
      context,
      capacity: capacity,
      warnings: warnings,
    );
    ...
    final finish = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'Simpan data sekaligus selesaikan WO ini? WO yang selesai akan '
          'dikunci dan siap disinkronkan.',
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            ...
            child: const Text('Selesaikan WO'),
          ),
        ],
      ),
    );
  ...
  Future<void> _persist(bool finish) async {
    ...
    try {
      final saved = _buildItem(finish: finish);
      ...
      if (finish) {
        await showDialog<void>(
          ...
          builder: (dialogContext) => AlertDialog(
            ...
            content: Text(
              'Semua data Inspeksi Gardu telah disimpan dan dikunci.\n\n'
              '${wo.kodeWo}',
            ),
            actions: [
              FilledButton(
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            _measurement('02', 'Pengukuran WBP', true),
            const SizedBox(height: 18),
            _measurement('03', 'Pengukuran LWBP', false),
            const SizedBox(height: 18),
            _conditions(),
            const SizedBox(height: 18),
            _findings(),
  ...
  Widget _hero() => Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [blue, darkBlue]),
          ...
          children: [
            Row(
              children: [
                const Expanded(
                  child: Text(
                    'WORK ORDER INSPEKSI GARDU',
                    style: TextStyle(
                      color: Color(0xFFB9D9EA),
                      fontSize: 10,
            ...
            Text(
              wo.nomorGardu.isEmpty ? '-' : wo.nomorGardu,
            ...
            Container(
              ...
              decoration: BoxDecoration(
                border: Border.all(color: const Color(0xFF70A8CC)),
            ...
            const SizedBox(height: 20),
            Text(
              '${wo.ulp} • ${wo.penyulang} • ${wo.tanggal}',
  ...
  Widget _statusChip() => Container(
        ...
        child: Text(
          WoInsdu.normalisasiStatus(wo.statusWo),
          style: const TextStyle(
        ...
        Padding(
          ...
          child: Column(
            children: [
              Row(
                children: [
                  Expanded(child: _readonly('PENYULANG', wo.penyulang)),
                  const SizedBox(width: 12),
                  Expanded(child: _readonly('SECTION', wo.section)),
                ],
              ),
              const SizedBox(height: 15),
              DropdownButtonFormField<int>(
                key: ValueKey('capacity-$_capacity'),
                initialValue: _capacity,
                isExpanded: true,
                decoration: _decoration('KAPASITAS'),
                hint: const Text('Pilih kapasitas trafo'),
                items: WoInsdu.kapasitasOptions
                    .map(
                      (value) => DropdownMenuItem(
                        value: value,
                        child: Text('$value kVA'),
                      ),
                    )
                    .toList(),
                onChanged:
                    readOnly ? null : (value) => setState(() => _capacity = value),
              ),
              if (_capacity != null)
                Padding(
                  ...
                  child: Align(
                    ...
                    child: Text(
                      'Arus maksimal per fasa '
                      '${WoInsdu.maximumPhaseCurrent(_capacity!).toStringAsFixed(1).replaceAll('.', ',')} A',
                      style: const TextStyle(
                        color: muted,
                        fontSize: 11,
      ...
      Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _timeRow(
              wbp ? 'Waktu Penginputan WBP' : 'Waktu Penginputan LWBP',
              time,
            ),
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    distance == null
                        ? '—'
                        : '${distance.toStringAsFixed(1).replaceAll('.', ',')} m',
                    style: const TextStyle(
                      color: navy,
                      fontSize: 28,
                      fontWeight: FontWeight.w900,
            ...
            const Divider(),
            _groupHeader('A', 'Beban Utama', 'ARUS'),
            _numericGrid(wbp ? wbpLoad : lwbpLoad, wbp),
            const SizedBox(height: 20),
            const Divider(),
            _groupHeader('V', 'Tegangan', 'VOLT'),
  ...
  Widget _locationHeader(bool wbp) => Container(
        ...
        decoration: BoxDecoration(
          color: cyan,
          borderRadius: BorderRadius.circular(15),
          border: Border.all(color: const Color(0xFF7FC8D8)),
        ),
        child: Row(
          children: [
            Container(
              ...
              child: Stack(
                alignment: Alignment.center,
                children: [
                  const Icon(Icons.gps_fixed_rounded, color: blue, size: 32),
                  Container(
                    width: 7,
                    height: 7,
            ...
            Expanded(
              child: Text(
                wbp
                    ? 'Lokasi Penginputan WBP'
                    : 'Lokasi Penginputan LWBP',
                style: const TextStyle(
                  color: navy,
                  fontWeight: FontWeight.w900,
                ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    value.isEmpty ? 'Belum direkam' : value,
                    style: const TextStyle(
                      color: navy,
                      fontSize: 12,
                      fontWeight: FontWeight.w900,
                ...
                .map(
                  (entry) => SizedBox(
                    ...
                    child: TextField(
                      ...
                      decoration: _decoration(entry.value).copyWith(
                        errorText: _decimalError(entry.key),
                      ),
                    ),
                  ),
                )
        ...
        Padding(
          ...
          child: Column(
            children: conditionLabels.entries.map((entry) {
              final options = entry.key.startsWith('jumperan')
                  ? jumperOptions
                  : coverOptions;
