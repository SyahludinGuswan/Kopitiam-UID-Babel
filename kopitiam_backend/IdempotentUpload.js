...
function syncTemuanInspeksiIdempotent_(token, incoming) {
  ...
  if (isC4a) { if (!/^PEG-[A-Z0-9]+\.TO-[0-9]{3}$/.test(code)) return fail_("FINDING_CODE_INVALID", "Kode Temuan C4A tidak valid."); incoming["Kode WO"] = ""; incoming["Jenis WO"] = ""; }
  ...
  var master = validateFindingMaster_(object, tier, finding, priority); if (!master.success) return master;
  ...
  try { point = validateCoordinate_(safeText_(incoming["Koordinat Temuan"], 80)); primaryPrepared = preparePhoto_(incoming.fotoTemuanBase64, "Foto Temuan"); environmentPrepared = preparePhoto_(incoming.fotoLingkunganBase64, "Foto Lingkungan"); } catch (_) { return fail_("INPUT_INVALID", "Koordinat atau file foto tidak valid."); }
  var lock = LockService.getScriptLock(), created = [];
  lock.waitLock(30000);
  try {
    var context = isC4a ? c4aContext_(auth.sesi, incoming) : woContext_(auth.sesi, kodeWo, true); if (!context.success) return context;
