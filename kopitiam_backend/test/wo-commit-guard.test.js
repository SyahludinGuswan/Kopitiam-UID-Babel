...
test('canonical comparison accepts equivalent numeric Sheet display values', () => {
  ...
  assert.equal(api.woCommitEqual_('  Progress   Pekerjaan ', 'progress pekerjaan'), true);
  assert.equal(api.woCommitEqual_('Koba', 'Toboali'), false);
});

test('unmapped local fields fail instead of being silently ignored', () => {
  const api = load();
  assert.throws(() => api.woCommitExpected_(['Kode WO', 'ULP', 'Tanggal'], {
    'kode wo': 'WO-1', ulp: 'Koba', tanggal: '13 September 2026', 'kolom baru': 'nilai',
...
test('Drive verification checks MIME, byte count, and digest', () => {
  ...
  const file = { getId: () => 'file-1', getName: () => 'photo.jpg', getUrl: () => 'https://drive/photo', getBlob: () => ({ getContentType: () => 'image/jpeg', getBytes: () => bytes }) };
...
test('Drive digest mismatch blocks acknowledgement', () => {
  const api = load();
  const file = { getBlob: () => ({ getContentType: () => 'image/jpeg', getBytes: () => [1, 2, 3] }) };
  assert.throws(() => api.woCommitVerifyPhoto_({ file }, { bytes: [1, 2, 3], digest: 'wrong' }), (error) => error.woCommitCode === 'WO_PHOTO_DIGEST_MISMATCH');
});

test('receipt binds WO identity, payload digest, and verified photo', () => {
  const api = load();
  const item = { code: 'WO-1', normalized: { ulp: 'Koba', tanggal: '13 September 2026' } };
  const receipt = api.woCommitReceipt_(item, { 'status wo': 'Selesai' }, { receipt: { fileId: 'file-1', digest: 'abc' } });
