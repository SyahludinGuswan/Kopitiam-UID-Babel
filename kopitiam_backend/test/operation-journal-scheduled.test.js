...
const selectedSource=fs.readFileSync(path.join(__dirname,'../ZZZZZZZZZZZZZZZZZZZZZ_Rel07BehaviorGuard.js'),'utf8');
...
test('selected stale-lease implementation refreshes state under lock',()=>{assert.match(selectedSource,/operationJournalSweepStaleLeases_\(\)/);assert.match(selectedSource,/getDisplayValues\(\)/);assert.match(selectedSource,/operationJournalFind_\(freshSource, operationId\)/);assert.match(selectedSource,/getScriptLock\(\)/);});
